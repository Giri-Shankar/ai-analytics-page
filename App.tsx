...
const App: React.FC = () => {
  ...
  const parseCSVContent = useCallback((content: string): Promise<Record<string, any>[]> => {
  ...
  const handleDataLoaded = useCallback(async (files: ParsedFileData[]) => {
    try {
      // Parse all CSV files
      const parsePromises = files.map(file => parseCSVContent(file.content));
